...
//得到数据库中所有商品
$sql = "select p.*,c.cName from stw_pro as p join stw_cate c on p.cId=c.id {$where}  ";
...
$sql = "select p.*,c.cName from stw_pro as p join stw_cate c on p.cId=c.id {$where} {$orderBy} limit {$offset},{$pageSize}";
$rows = fetchAll($sql);
?>
<!doctype html>
<html>
