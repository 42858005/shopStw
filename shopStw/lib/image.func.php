...
function verifyImage($type = 1, $length = 4, $pixel = 0, $line = 0, $sess_name = "verify") {
    ...
    $chars = buildRandomString($type,$length);//生成随机
    
    $_SESSION[$sess_name] = $chars;
    $fontfiles = array("MSYH.TTC","MSYHBD.TTC","SIMLI.TTF","SIMSUN.TTC","SIMYOU.TTF","STZHONGS.TTF");
    for ($i = 0; $i < $length; $i++) {
        $size = mt_rand(14,18);
        $angle = mt_rand(-15,15);
        $x = 5+$i*$size;
    ...
    if ($pixel) {
        for ($i = 0; $i < $pixel; $i++) {
            imagesetpixel($image, mt_rand(0,$width-1), mt_rand(0,$height), $black);
        }
    }
    if ($line) {
        for ($i = 0; $i < $line; $i++) {
            $color = imagecolorallocate($image, mt_rand(50,90), mt_rand(80,200), mt_rand(90,180));
            imageline($image, mt_rand(0,$width-1), mt_rand(0,$height), mt_rand(0,$width-1), mt_rand(0,$height), $color);
        }
    }
...
function waterText($filename, $text = "stw516", $fontfile = "MSYH.TTC"){
    ...
    $x = ceil (($src_w- $textWidth) / 2);
    $y = ceil (($src_h- $fontHeight) / 2);
    $angle = mt_rand(0,360);
